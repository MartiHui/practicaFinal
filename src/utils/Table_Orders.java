...
public class Table_Orders {
	...
	public Table_Orders(Object[] cabecera, Class<?>[] tipos, Integer[] medidas, Color par, Color impar, Color sel) {
		...
		tabla = new JTable(){
			...
			public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
		        ...
		        if (!isRowSelected(row)) {
		        	...
		        	if ((Boolean) getValueAtB(row, 5) == true) {
		        		c.setBackground(Color.green);
		        	} else {
		        		c.setBackground(row%2==0? colorPar:colorImpar);
		        	}
		        } else {
					c.setBackground(colorSel);
				}
	...
	public Object getValueAtB(int fileIndex, int columnIndex) {
		if (fileIndex<0) return null;
		return modelo.getValueAt(tabla.getRowSorter().convertRowIndexToModel(fileIndex), columnIndex); 
	}
	public void vaciar() {
